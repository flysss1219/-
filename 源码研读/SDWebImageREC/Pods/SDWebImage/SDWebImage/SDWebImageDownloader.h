...
typedef NS_OPTIONS(NSUInteger, SDWebImageDownloaderOptions) {
     ...
     */
    SDWebImageDownloaderLowPriority = 1 << 0,//图片下载设置低优先级
    
    /**
     * This flag enables progressive download, the image is displayed progressively during download as a browser would do.
     */
    SDWebImageDownloaderProgressiveDownload = 1 << 1,//分进度下载图片

    /**
     * By default, request prevent the use of NSURLCache. With this flag, NSURLCache
     * is used with default policies.
     */
    SDWebImageDownloaderUseNSURLCache = 1 << 2,//设置了该选项则使用NSURLCache缓存策略

    /**
     * Call completion block with nil image/imageData if the image was read from NSURLCache
     * (to be combined with `SDWebImageDownloaderUseNSURLCache`).
     */
    SDWebImageDownloaderIgnoreCachedResponse = 1 << 3,//如果图片从NSURLCache读取，则回到block中的image和imageData置为nil
    
    /**
     * In iOS 4+, continue the download of the image if the app goes to background. This is achieved by asking the system for
     * extra time in background to let the request finish. If the background task expires the operation will be cancelled.
     */
    SDWebImageDownloaderContinueInBackground = 1 << 4,//在iOS4之后，如果设置了此选项则在后台依然下载图片，申请额外的时间来进行后台下载

    /**
     * Handles cookies stored in NSHTTPCookieStore by setting 
     * NSMutableURLRequest.HTTPShouldHandleCookies = YES;
     */
    SDWebImageDownloaderHandleCookies = 1 << 5,// 处理存储在NSHTTPCookieStore中的cookies

    /**
     * Enable to allow untrusted SSL certificates.
     * Useful for testing purposes. Use with caution in production.
     */
    SDWebImageDownloaderAllowInvalidSSLCertificates = 1 << 6,//允许非信任的SSL证书

    /**
     * Put the download in the high queue priority and task priority.
     */
    SDWebImageDownloaderHighPriority = 1 << 7,//设置图片下载高优先级
    
    /**
     * Scale down the image
     */
    SDWebImageDownloaderScaleDownLargeImages = 1 << 8,//对大图片进行缩放
};
...
typedef NS_ENUM(NSInteger, SDWebImageDownloaderExecutionOrder) {
     ...
     */
    SDWebImageDownloaderFIFOExecutionOrder,//按照队列先进先出的顺序下载

    /**
     * All download operations will execute in stack style (last-in-first-out).
     */
    SDWebImageDownloaderLIFOExecutionOrder //按照栈后进先出进行下载
};
 ...
 */
@property (assign, nonatomic) NSInteger maxConcurrentDownloads;//设置最大并发数量

/**
 * Shows the current amount of downloads that still need to be downloaded
 */
@property (readonly, nonatomic) NSUInteger currentDownloadCount;//获取当前下载的数量

/**
 *  The timeout value (in seconds) for the download operation. Default: 15.0.
 */
@property (assign, nonatomic) NSTimeInterval downloadTimeout;//下载时间 默认15秒
 ...
 */
@property (strong, nonatomic, nullable) NSURLCredential *urlCredential;//设置身份认证
 ...
 */
- (void)setSuspended:(BOOL)suspended;//设置下载队列是否挂起
 ...
 */
- (void)createNewSessionWithConfiguration:(nonnull NSURLSessionConfiguration *)sessionConfiguration;//强制给self设置一个新的NSURLSession
 ...
 */
- (void)invalidateSessionAndCancel:(BOOL)cancelPendingOperations;//取消operation并且session设置为Invalidates
